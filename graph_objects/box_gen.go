...
type Box struct {
    ...
    // Boxmean 
    // enumerated If *true*, the mean of the box(es)' underlying distribution is drawn as a dashed line inside the box(es). If *sd* the standard deviation is also drawn. Defaults to *true* when `mean` is set. Defaults to *sd* when `sd` is set Otherwise defaults to *false*. 
    ...
    // Hoveron 
    // flaglist Do the hover effects highlight individual boxes  or sample points or both? 
    ...
    // Orientation 
    // enumerated Sets the orientation of the box(es). If *v* (*h*), the distribution is visualized along the vertical (horizontal). 
