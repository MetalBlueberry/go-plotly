...
type Mesh3d struct {
    ...
    // Delaunayaxis 
    // enumerated Sets the Delaunay axis, which is the axis that is perpendicular to the surface of the Delaunay triangulation. It has an effect if `i`, `j`, `k` are not provided and `alphahull` is set to indicate Delaunay triangulation. 
    ...
    // Intensitymode 
    // enumerated Determines the source of `intensity` values. 
