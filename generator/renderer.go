...
func (r *Renderer) CreatePlotGo(dir, cdnUrl, tag string) error {
	src := &bytes.Buffer{}
	err := r.WritePlotGo(src, cdnUrl, tag)
...
func (r *Renderer) WritePlotGo(w io.Writer, cdnUrl string, tag string) error {
	// inject the basehtml with the correct plotly cdn reference
	data := struct {
		CDN      string
		Template string
		Tag      string
	}{CDN: cdnUrl, Template: fmt.Sprintf("fmt.Sprintf(`%s`, head)", generationTemplate), Tag: tag}
